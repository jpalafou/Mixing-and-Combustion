...
module global
  ...
  ! INPUTS:
  ! domain parameters
  real(dp), parameter :: Dxi = 0.000003d0 ! 0.0000015d0 ! mesh size in x (initial guess)
  real(dp), parameter :: Dyi = 0.04d0 ! mesh size in y (initial guess)
  real(dp), parameter :: xf = 2.0d0 ! final position in x
  real(dp), parameter :: yu = 12.0d0 ! upper y boundary
  real(dp), parameter :: yl = -12.0d0 ! lower y boundary
  ...
  real(dp) :: T_ref = 300.d0 ! free stream temperature at y = yu, [K]
  real(dp) :: u_ratio = 2.0d0 ! u_{\infinity} / u_{-\infinity}
